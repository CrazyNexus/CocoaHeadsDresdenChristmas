...
- (id)init
{
    ...
    if (self) {
        ...
        [RACObserve(self, sectionsDatasource) subscribeNext:^(id x) {
...
- (void)registerCellReuseIdentifier:(NSString *)reuseIdentifier forDataObject:(Class)classType setupBlock:(CHDDataSourceManagerCellSetupBlock)setupBlock {
    [self registerNib:nil forCellReuseIdentifier:reuseIdentifier forDataObject:classType setupBlock:setupBlock];
}
...
- (id)dataForIndexPath:(NSIndexPath *)indexPath {
    return [self.sectionsDatasource[indexPath.section] objectAtIndex:indexPath.row];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [self.sectionsDatasource[section] count];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSObject *data          = [self.sectionsDatasource[indexPath.section] objectAtIndex:indexPath.row];
